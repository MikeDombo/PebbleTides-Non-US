...
var version = '2.12';
var allTideData;
if (typeof localStorage.allTideData !== "undefined" && localStorage.allTideData !== null && localStorage.allTideData !== "") {
    allTideData = JSON.parse(localStorage.allTideData);
}
if (typeof allTideData === "undefined") {
    allTideData = {
        location1: {
            displayName: "",
            lookupName: ""
...
if (allTideData.units === null || typeof allTideData.units === "undefined") {
    allTideData.units = "M";
}
...
function substringSearch(search, start, end){
	var startIndex = search.indexOf(start);
	if(startIndex < 0){
		return "";
	}
	
	var substr = search.substring(startIndex + start.length);
	var endIndex = substr.indexOf(end);
...
function parseTide(tides, index, loc_name) {
    //setup Vars
    console.log("finding tide in parseTide");
    ...
    var minute;
    if ((Math.round(Math.abs(tideTimemin)) == 1 || Math.round(Math.abs(tideTimemin % 60)) == 1)) {
        minute = " minute";
    } else {
        minute = " minutes";
    }
    if (Math.abs(tideTimehr) > 1) {
        hour = " hours";
    } else {
        hour = " hour";
    }
    ...
    if (allTideData.hourFormat == "12h" && date.getHours() > 12) {
        ...
        tideTimeStr = date.getHours() + ":" + minutes;
    }
...
function setData(response, loc_name) {
    var high_tides = substringSearch(response, "var high_tides = Array(", "var low_tides = Array(");
    var low_tides = substringSearch(response, "var low_tides = Array(", "var hrdiff");
    loc_name = substringSearch(response, "Today's tide times for ", "</h2>");
    console.log(high_tides + " " + low_tides);
    high_tides = high_tides.substring(0, high_tides.length - 9);
    high_tides = high_tides.split(", ");
    low_tides = low_tides.substring(0, low_tides.length - 9);
    low_tides = low_tides.split(", ");
	
    var currTime = Math.round(Date.now() / 1000);
    high_tides[high_tides.length] = currTime;
    high_tides.sort(function(a, b) {
        return a - b;
    });
    low_tides[low_tides.length] = currTime;
    low_tides.sort(function(a, b) {
        return a - b;
    });
    ...
    var tideMenu = new UI.Menu({
        sections: [{
            title: 'Tides in ' + loc_name,
            items: [{
				title: 'High Tide',
				subtitle: times[0]
			},
    ...
    tideMenu.on('select', function(e) {
        parseTide(tides, e.itemIndex, loc_name);
    });
    tideMenu.show();
}
...
function getTides(zip) {
    ...
    ajax({
        ...
        function(result) {
            ...
            ajax({
                    url: 'http://mikedombrowski.com/analytics/gather.php?location=' + zip + '&US=FALSE&ID=' + Pebble.getAccountToken() + '&settings=' + encodeURIComponent(JSON.stringify(allTideData)) + "&ver=" + version
                },
                function(data) {},
                function(error) {});
        ...
        if (locations[e.itemIndex].title == 'About') {
            ...
            var settMenu = new UI.Menu({
                sections: [{
                    title: 'App Settings',
                    items: [{
                            title: 'Current (' + allTideData.hourFormat + ')',
                            subtitle: '12 or 24 Hour Time'
                        },
                        {
                            title: 'Current (' + allTideData.units + ')',
                            subtitle: 'Tide Height Units (Ft/M)'
                        }
                    ]
                }]
            });
            settMenu.on('select', function(e) {
                if (e.itemIndex === 0) {
                    if (allTideData.hourFormat == '12h') {
                        allTideData.hourFormat = '24h';
                    } else {
                        allTideData.hourFormat = '12h';
                    }
                    settMenu.items(0, [{
                        title: 'Current (' + allTideData.hourFormat + ')',
                        subtitle: '12 or 24 Hour Time'
